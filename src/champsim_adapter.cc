...
public:
  ...
  /** @brief Treat prefetches exactly as loads
   */
...
void printCacheStats() {
  assert(L1D != nullptr);
  cout << L1D->NAME << ": ";
  cout << " load: " << L1D->sim_access[0][0] << " hit: " << L1D->sim_hit[0][0]
       << " miss: " << L1D->sim_miss[0][0] << endl;
  L1D->impl_prefetcher_final_stats();
}

static void initializeL1DCache() {
  assert(L1D == nullptr);
  assert(dummyMem == nullptr);

  // Inform the cache that the warmup is complete
  std::fill_n(warmup_complete,
              sizeof(warmup_complete) / sizeof(warmup_complete[0]), 1);

  dummyMem = new DummyMemoryRequestConsumer();
  L1D = new CACHE("L1D",           // NAME
                  1.0,             // freq_scale
                  4,               // fill_level
                  64,              // NUM_SET
                  12,              // NUM_WAY
...
void initChampSim(PrefetchAddressGenerator &gen) {
  ...
  // Give the prefetcher a head start:
  // run the mem hierarchy for 2 cycles:
  // 1. the prefetcher issues the first request
  // 2. the line is placed in the cache
  L1D->_operate();
  L1D->_operate();
}
...
void load(uint64_t addr) {
  ...
  // We need `_operate()` and not just `operate()` to advance the cache's
  // perception of time aka cycle counter
  L1D->_operate();
}
